...
describe('Should get diagnostics', () => {
  ...
  it('Diagnoses invalid keys and references', async () => {
    await testDiagnostics(docUri, [
      { message: 'Authentication:Authenticatio is not a valid path.',
    ...
    await testDiagnostics(dailyTimeSheetsUri, [
      { message: 'DailyTimeSheets_Headin is not a valid path.',
      ...
      { message: '- .workspace.workspaces[\'DailyTimeSheets\'].layout.heading is a required property',
      range: toRange(6, 3, 6, 10), severity: vscode.DiagnosticSeverity.Warning, source: 'vscode-lsp' },
      { message: '- .workspace.workspaces[\'DailyTimeSheets\'].layout.rows is a required property',
      range: toRange(6, 3, 6, 10), severity: vscode.DiagnosticSeverity.Warning, source: 'vscode-lsp' }
    ]);
    ...
    await testDiagnostics(workspaceUri, [
      ...
      { message: '- .workspace.workspaces[\'MyReports\'].name is a required property',
      range: toRange(164, 5, 164, 15), severity: vscode.DiagnosticSeverity.Warning, source: 'vscode-lsp' },
      { message: '- .workspace.workspaces[\'MyReports\'].title is a required property',
      range: toRange(164, 5, 164, 15), severity: vscode.DiagnosticSeverity.Warning, source: 'vscode-lsp' },
      { message: '- .workspace.workspaces[\'MyReports\'].dataBindings is a required property',
      range: toRange(164, 5, 164, 15), severity: vscode.DiagnosticSeverity.Warning, source: 'vscode-lsp' },
      { message: '- .workspace.workspaces[\'MyReports\'].layout is a required property',
      range: toRange(164, 5, 164, 15), severity: vscode.DiagnosticSeverity.Warning, source: 'vscode-lsp' }
    ]);
  });
});
